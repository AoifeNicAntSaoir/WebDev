...
$config = parse_ini_file("../../bookshopConfig.ini");
...
$dbcnx = @mysql_connect($host, $username);
if (!$dbcnx) {
...
<form action="selectListbox2.php" method="POST">
...
echo(" and to make sure that correct record was selected for we output the status " .$row[quantity]. "<br>");
 echo("select a book"); 
 $result = @mysql_query("SELECT * FROM books");
echo("<select name='bookname'>");
